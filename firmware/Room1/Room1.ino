...
void setup(void) {
  ...
  Xbee.println("Hello from ROOM_1 ");
  delay(1000);
}
...
void loop(void) {
  ...
  if (millis() - count > 30000) {
    //Packet Structure: ID_Room:Temp:Humidity:Pressure
  ...
  //********************************************// ZIGBEE RX COMMUNICATION MANAGEMENT
...
void parseCommand(String command) {
  ...
  if (command.indexOf("r1") == -1 && command.indexOf("room1") == -1) {
    return;
  }
  ...
  if (command.indexOf("heater") >= 0) {
    if (command.indexOf("off") >= 0) {
      ...
      Xbee.println("Heater_R1 = 0");        //Back command to Gateway [OFF]
      ...
      Xbee.println("Heater_R1 = 1");        //Back command to Gateway [ON]
    }
    return;
  }
  ...
  if (command.indexOf("light") >= 0) {
    if (command.indexOf("off") >= 0) {
      ...
      Xbee.println("Light_R1 = 0");         //Back command to Gateway [OFF]
    } else if (command.indexOf("on") >= 0) {
      digitalWrite(LED_PIN, HIGH);
      Serial.println(" --> LIGHT ON <-- "); 
      Xbee.println("Light_R1 = 1");         //Back command to Gateway [ON]
    }  
    return;
  }
